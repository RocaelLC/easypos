...
import type { CartItem, Product } from "@/lib/posTypes";
...
export default function POSPage() {
  ...
  function onProductClick(p: Product) {
    if (p.modifiers && p.modifiers.length > 0) setCustomizing(p);
    else {
      const item: CartItem = {
        ...
        <section className="col-span-12 md:col-span-7 grid grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((p) => (
            ...
            >
              ...
              <div className="text-sm text-neutral-400 mt-1">{money(p.basePrice)}</div>
              {p.modifiers?.length ? (
        ...
        <aside className="col-span-12 md:col-span-3 border-l border-neutral-800 md:pl-3">
          ...
          <div className="mt-4 border-t border-neutral-800 pt-3">
            ...
            <button
              ...
              onClick={() => alert("Siguiente: flujo de cobro + venta offline")}
            >
              Cobrar
            </button>
          </div>
