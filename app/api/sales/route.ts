...
export async function POST(req: Request) {
  try {
    ...
    // 1) Idempotencia
    const exists = await sales.findOne({ clientSaleId, createdByUid: user.uid });
    if (exists) {
    ...
    // ✅ 2.1) Movimiento de cartera por venta (idempotente por clientSaleId / usuario)
    try {
      ...
      await db.collection("wallet_movements").updateOne(
        { kind: "sale", "origin.clientSaleId": String(clientSaleId), createdByUid: user.uid },
        {
          $setOnInsert: {
            kind: "sale",
            direction: "in",
    ...
    try {
      ...
      for (const sold of items) {
        ...
        const prod = await productsCol.findOne({ ownerUid: user.uid, productId: sold.productId });
